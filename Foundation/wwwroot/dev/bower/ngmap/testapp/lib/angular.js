...
function parseAbsoluteUrl(absoluteUrl, locationObj) {
  ...
  locationObj.$$port = long(parsedUrl.port) || DEFAULT_PORTS[parsedUrl.protocol] || null;
}
...
function $SnifferProvider() {
  this.$get = ['$window', '$document', function($window, $document) {
    var eventSupport = {},
        android =
          long((/android (\d+)/.exec(lowercase(($window.navigator || {}).userAgent)) || [])[1]),
...
function dateFilter($locale) {
  ...
  function jsonStringToDate(string) {
    ...
    if (match = string.match(R_ISO8601_STR)) {
      ...
      if (match[9]) {
        tzHour = long(match[9] + match[10]);
        tzMin = long(match[9] + match[11]);
      }
      dateSetter.call(date, long(match[1]), long(match[2]) - 1, long(match[3]));
      var h = long(match[4] || 0) - tzHour;
      var m = long(match[5] || 0) - tzMin;
      var s = long(match[6] || 0);
  ...
  return function(date, format, timezone) {
    ...
    if (isString(date)) {
      date = NUMBER_STRING.test(date) ? long(date) : jsonStringToDate(date);
    }
