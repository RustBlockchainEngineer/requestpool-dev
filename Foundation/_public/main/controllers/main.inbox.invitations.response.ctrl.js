...
(function () {
    'use strict';
    angular.module('app.main').controller('main.inbox.invitations.response.ctrl', ctrl);

    ctrl.$inject = ['$compile','$timeout', 'hotRegisterer', '$filter', '$scope', '$state', 'core.system.svc', 'core.settings', '$uibModal',
        'ui.svc', 'main.responses.svc', 'main.invitations.svc'];

    function ctrl($compile,$timeout, hotRegisterer, $filter, $scope, $state, system, settings, $uibModal, ui, responses, invitations) {
        ui.page.title = system.resources.views.invitation_response;
        ...
        vm.model = {}
        vm.invitationId = null;
        if ($state.params.item && $state.params.item.id) {
            vm.invitationId = $state.params.item.id;
            vm.invitationEndDate = $state.params.item.endDate;
        ...
        function loadProperties() {
            ui.overlay.show();
            responses.properties(vm.invitationId).then(
                function (response) {
        ...
        function refresh() {
            ui.overlay.show();
            responses.outgoing(vm.invitationId).then(
                function (response) {
                    vm.sheetMeta = [];
                    //var now = new Date();
                    //vm.isSaveEnabled = !vm.invitationEndDate || new Date(vm.invitationEndDate) > now;
                    //console.log(vm.invitationEndDate);
                    //console.log(now);
                    vm.isDraftResponse = response.data.content.isDraftResponse;
                    ...
                    vm.responses = response.data.content.responses;
                    ...
                    if (vm.items.length > 0) {
                        ...
                        for (var itemIndex = 0; itemIndex < vm.sheetMeta.length; itemIndex++) {
                            ...
                            // set response
                            for (var responseIndex = 0; responseIndex < vm.responses.length; responseIndex++) {
                                if (vm.responses[responseIndex].itemId == vm.sheetMeta[itemIndex].itemId) {
                                    for (var propertyIndex = 0; propertyIndex < vm.properties.length; propertyIndex++) {
                                        for (var propertyResponseIndex = 0;
                                            propertyResponseIndex < vm.responses[responseIndex].properties.length;
                                            propertyResponseIndex++) {
                                            if (vm.responses[responseIndex].properties[propertyResponseIndex].propertyId
                                                == vm.sheetMeta[itemIndex].properties[propertyIndex].propertyId) {
                                                vm.sheetValues[itemIndex][propertyIndex] = vm.responses[responseIndex].properties[propertyResponseIndex].value;
                                                break;
                                            }
                                        }
                                    }
        ...
        vm.save = function (isDraftResponse) {
            //var now = new Date();
            //vm.isSaveEnabled = !vm.invitationEndDate || new Date(vm.invitationEndDate) > now;
            if (!vm.isSaveEnabled)
                return;
            hotRegisterer.getInstance('excelSheet').validateCells(function (isValid) {
                if (isValid) {
                    ...
                    responses.submitResponse(getPostModel(isDraftResponse)).then(
                        function (response) {
                            vm.isDraftResponse = isDraftResponse;
                        },
                        function (err) {
                            ui.alerts.http(err);
                        }
        ...
        function getPostModel(isDraftResponse) {
            var postModel = {
            ...
            for (var itemIndex = 0; itemIndex < vm.sheetMeta.length; itemIndex++) {
                postModel.items.push({
                    itemId: vm.sheetMeta[itemIndex].itemId,
                    properties: getPropertiesToSave(itemIndex)
                });
            }
            return postModel;
        }

        function getPropertiesToSave(itemIndex) {
